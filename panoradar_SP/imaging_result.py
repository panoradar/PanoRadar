...
def imaging_frame_from_traj(root_folder_name: str, traj_name: str, frame_ind: int, out_path: str):
    """
    Processes raw signal data for one frame in a trajectory and plots the resulting 2D floormap

    Args:
        root_folder_name: name of the root folder holding the raw signal data
        traj_name: name of the trajectory that we are imaging
        frame_ind: the frame number that we want to visualize
    """

    # Get frames and params from dataset
    radar_frames, lidar_frames, _, params = get_all_from_dataset(traj_name, root_folder_name)

    # Get motion parameters from dataset
    try:
        motion = get_motion_params(traj_name, root_folder_name)
    except OSError as e:
        # If there is no motion.npz then it is a static trajectory
        # Set all motion params to 0
        motion = {
