...
from panoradar_SP.dataset import get_all_from_dataset, list_all_trajs_complete
import panoradar_SP.radar_imaging_3d as imaging_3d
from panoradar_SP.velocity_estimation import estimate_v_line_detect

def estimate_params_for_trajectory(traj_name: str, root_folder_name: str):
    """Estimate the motion parameters for one whole trajectory.
       Also save the lidar ground truth for evaluation.
    Args:
        traj_name: name of the trajectory to be saved
        root_folder_name: name of the root folder that holds the SP data
    ...
    # read the trajectory
    print('Loading data')
    radar_frames, lidar_frames, imu_frames, params = get_all_from_dataset(
        traj_name, root_folder_name, False
    )

    # If trajectory is static, then save all 0s
    if "static" in traj_name:
        np.savez(
            f'{root_folder_name}/{traj_name}/motion_output.npz',
            v_esti=np.zeros(len(radar_frames)),
            theta_v_esti=np.zeros(len(radar_frames)),
            delta_s0_esti=np.zeros(len(radar_frames)),
            omega_imu=np.zeros(len(radar_frames)),
            v_gt=np.zeros(len(radar_frames)),
            theta_v_gt=np.zeros(len(radar_frames)),
            delta_s0_gt=np.zeros(len(radar_frames)),
            omega_gt=np.zeros(len(radar_frames)),
        )
        return
    ...
    for pick_frame in tqdm(range(len(radar_frames))):
        ...
        # estimate motion
        params_esti, *_ = estimate_v_line_detect(
            signal_bf,
            azimuth_compen,
            len(beam_angles),
            params,
...
def estimate_whole_dataset(root_folder_name: str):
    """
    Estimates the motion parameters for all trajectories for the complete dataset
    Calls 'estimates_params_for_trajectory' for each trajectory in the dataset

    Args:
        root_folder_name: the name of the root folder that holds the data
    ...
    all_traj_names = list_all_trajs_complete(root_folder_name)
    ...
    for i, traj_name in enumerate(all_traj_names):
        ...
        estimate_params_for_trajectory(traj_name, root_folder_name)
