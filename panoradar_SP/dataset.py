...
For the details of each dataset, please refer to dataset_description.txt
"""
import numpy as np
import cupy as cp
...
class RadarFrames:
    ...
    def __init__(
        ...
        self.radar_end_frame_sep = self.__find_radar_end_frame_sep(
            imu_obj.imu_frames, imu_obj.frame_sep_ids
        )

    def __find_radar_end_frame_sep(
        self, imu_frames: np.ndarray, imu_frame_sep: np.ndarray
    ) -> np.ndarray:
    ...
    def get_a_frame_data(self, frame_id: int, pad: bool = True) -> Tuple[np.ndarray, cp.ndarray]:
        """Get the data of the radar frame for a cycle.
        Args:
            frame_id: frame index, which frame do you want, start from 0
            pad: whether we pad some chirps at the beginning and the end.
                Note that real time series signal is used instead of the
                old circular padding method.
        Returns:
        ...
        frame = (
            self.radar_frames[st_ind - self.half_syn_ante : end_ind + self.half_syn_ante]
            if pad
            else self.radar_frames[st_ind:end_ind]
        )
...
def _ourlier_filtering(
    in_array: np.ndarray, jitter_thres: float, assign_neighbor: int
) -> np.ndarray:
...
def _apply_start_beam_offset(
    radar_sep: np.ndarray,
    lidar_sep: np.ndarray,
    imu_sep: np.ndarray,
    lidar_transform: np.ndarray,
    st_beam_offset: int,
) -> np.ndarray:
    """Apply a start beam offset to all the data frames. Change inplace
    Args:
        st_beam_offset: offset value in percentage, between 0~100
    Return:
        new_lidar_transform
    """
    ratio = st_beam_offset / 100

    for sep in (radar_sep, lidar_sep, imu_sep):
        frame_len = np.mean(np.diff(sep))
        offset = round(frame_len * ratio)
        sep += offset

    cos_th = np.cos(-2 * np.pi * ratio)  # ccw
    sin_th = np.sin(-2 * np.pi * ratio)  # ccw
    return np.array([[cos_th, -sin_th], [sin_th, cos_th]]) @ lidar_transform


# =========================  New Dataset on High Speed Platform  =========================
def get_all_from_dataset(
    dataset_name: str, root_folder_name: str, need_video: bool = False, st_beam_offset: int = 0
) -> Tuple[RadarFrames, LidarFrames, ImuFrames, Dict[str, Any]]:
    """Parse the new high rotary speed dataset.
    Args:
        dataset_name: the name of the dataset.
        root_folder_name: the name of the root folder that holds the SP data
        need_video: whether the return dict has the video raw data
    ...
    try:
        data_params = np.load(f'{root_folder_name}/{dataset_name}/data_params.npz', allow_pickle=True)
        radar_sep = data_params['radar_sep']
        lidar_sep = data_params['lidar_sep']
        imu_sep = data_params['imu_sep']
        video_sep = data_params['video_sep']
    ...
    radar_frames = np.load(f"{root_folder_name}/{dataset_name}/radar_data.npy", mmap_mode='r')
    radar_frames = radar_frames.reshape(radar_frames.shape[0], -1, radar_frames.shape[3])
    radar_ts = np.loadtxt(f"{root_folder_name}/{dataset_name}/radar_ts.txt")

    # read lidar data  # (frames, 64, 2048, 3)
    lidar_frames = np.load(f"{root_folder_name}/{dataset_name}/lidar_data.npy", mmap_mode='r')
    lidar_ts = np.loadtxt(f"{root_folder_name}/{dataset_name}/lidar_ts.txt")

    # read imu data (external rotation, i.e. delta omega in deg/s) (N,)
    imu_frames = np.load(f"{root_folder_name}/{dataset_name}/imu_data.npy", mmap_mode='r')
    imu_ts = np.loadtxt(f"{root_folder_name}/{dataset_name}/imu_ts.txt")
    ...
    # --- Data and Param ---
    transform = _apply_start_beam_offset(radar_sep, lidar_sep, imu_sep, transform, st_beam_offset)
    imu_frames = ImuFrames(imu_frames, imu_ts, imu_sep)
    radar_frames = RadarFrames(radar_frames, radar_ts, radar_sep, imu_frames, N_syn_ante)
    lidar_frames = LidarFrames(lidar_frames, lidar_ts, lidar_sep, transform)
    # video_frames = VideoFrames(video_frames, video_ts, video_sep)
...
def get_lidar_2d_projection(
    lidar_frame: np.ndarray, transform: np.ndarray, elevation: float
) -> np.ndarray:
...
def get_motion_params(
    dataset_name: str, root_folder_name: str, need_smooth: bool = True, st_beam_offset: int = 0
) -> Dict[str, np.ndarray]:
    ...
    """
    motion = np.load(f'{root_folder_name}/{dataset_name}/motion_output.npz')
...
def change_data_params(dataset_name: str, **kwargs: Dict):
    """
    Change the params for a certain trajectory
    Args:
        dataset_name: name of the trajectory we want to change parameters for
        **kwargs: the parameters we want to change
    """
    data_params = np.load(f'data/{dataset_name}/data_params.npz', allow_pickle=True)
    ...
    np.savez(f'data/{dataset_name}/data_params.npz', **data_params)


def list_all_trajs(traj_folders: List[str], root_folder_name: str) -> List[str]:
    """
    Return all the trajectories name in the given list of folders.
    Args:
        traj_folders: list of folders that contain many trajectories
        root_folder_name: name of the root folder that holds the raw signal data

    E.g. traj_folders = ['2023_02_25']
         traj_names = ['2023_02_25/001', '2023_02_25/002', ...]
    """
    if not isinstance(traj_folders, list):
        raise TypeError('traj_folders must be a list.')

    traj_names = []
    for traj_folder in traj_folders:
        traj_folder_path = Path(f'{root_folder_name}/{traj_folder}')
        traj_names_paths = list(traj_folder_path.iterdir())
        traj_names_paths.sort()

        for traj_names_path in traj_names_paths:
            if traj_names_path.is_file():
                continue
            traj_names.append(str(traj_names_path.relative_to(root_folder_name)))

    return traj_names

def list_all_trajs_complete(root_folder_name: str) -> List[str]:
    """
    Returns a list of all trajectory names from the complete data

    Args:
        root_folder_name: The name of the root folder of the signal data
    """

    # Get all dataset names/traj folders
    traj_folders = []
    traj_folders_path = Path(root_folder_name)
    traj_folders_abs_list = list(traj_folders_path.iterdir())

    for traj_folder in traj_folders_abs_list:
        if traj_folder.is_file():
            continue

        traj_folders.append(str(traj_folder.relative_to(root_folder_name)))
    
    return list_all_trajs(traj_folders, root_folder_name)