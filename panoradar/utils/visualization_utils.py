...
def lidar_npy_to_png(lidar_npy: np.ndarray, y_min=0.05, y_max=0.9) -> np.ndarray:
    """Convert the npy lidar depth file to colored png for labeling.
    Args:
        lidar_npy: (1, 64, 512) depth
    Returns:
        lidar_png: the (64, 512, 3) RGB image
    """
    y = lidar_npy[0].copy()  # (64, 512)
    mask = y < 0

    y[y > y_max] = y_max
    y[y < y_min] = y_min
    y_normalized = (y - y_min) / (y_max - y_min) * 255
    y_normalized = y_normalized.astype(np.uint8)
    y_normalized = 255 - y_normalized

    lidar_png = cv2.applyColorMap(y_normalized, cv2.COLORMAP_MAGMA)
    lidar_png[mask] = [255, 255, 255]  # change failure region to white
    lidar_png = cv2.cvtColor(lidar_png, cv2.COLOR_BGR2RGB)
...
def visualize_lidar_gt_with_seg(lidar_data, gt_seg, parameters):
    ...
    vis.create_window(
        window_name='Lidar with semantic', 
        width=1920, 
        height=569, 
        left=0, 
        top=0, 
...
def visualize_depth_pred_with_seg(depth_pred, seg_pred, parameters):
    ...
    vis = open3d.visualization.Visualizer()
    vis.create_window(
        window_name='Pred Depth with Semantic',
        width=1920,
        height=569,
        left=0,
        top=0,
    ...
    for r_ind in range(r_window_half, num_r - r_window_half):
        for ele_ind in range(ele_window_half, num_elevation - ele_window_half):
            for azi_ind in range(azi_window_half, num_azimuth - azi_window_half):
                ...
                if (
                    data[r_ind, ele_ind, azi_ind] >= threshold
                    and data[r_ind, ele_ind, azi_ind] > second_thresh
                ):
...
def visualize_rf_heatmap_with_cfar(rf, parameters):
    ...
    # Visualize
    vis = open3d.visualization.Visualizer()
    vis.create_window(window_name='cfar3d', 
                      width=1920, 
                      height=569, 
                      left=0, 
                      top=0,
