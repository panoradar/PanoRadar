...
class OBJVisualizer(Visualizer):
    ...
    def draw_instance_predictions(self, predictions):
        ...
        if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
            ...
            colors = [
                [x / 255 for x in self.color_palette[clss]] for clss in classes
    ...
    def draw_box(self, box_coord, alpha=0.5, edge_color="g", line_style="-"):
        ...
        if x1 < x0:
            ...
            width = (
                self.img_width - x0 + 10
            )  # Draw first rectangle to img_width+10 so the right edge does not show
...
def draw_range_image(
    ...
    sp_depth_values = np.sort(rf, 0)[-1, : ,:]
    black_indices = sp_depth_values < -0.1
    sp_depth = np.argmax(rf, 0) * 0.00390625 # finding the distance corresponding to the index of the peak signal
    sp_depth[black_indices] = 9.6
    sp_depth = np.expand_dims(sp_depth, 0)
    sp_png = lidar_npy_to_png(sp_depth)
    
    gt_depth_copy = gt_depth.copy().squeeze()
    mask = gt_depth_copy > 0
    gt_depth_copy[mask == 0] = np.nan

    ax[0].imshow(sp_png, aspect='auto')
    ax[0].axis('off')
    ax[0].text(0.99, 0.95, 'After SP', horizontalalignment='right', verticalalignment='top', transform=ax[0].transAxes, color='black', fontsize=20)
    ax[0].set_title('Frontal View', fontsize=25, weight='bold')
    ax[1].imshow(-pred_depth.squeeze(), aspect='auto', cmap='magma', vmin=-0.8, vmax=-0.05)
    ax[1].axis('off')
    ax[1].text(0.99, 0.95, 'After ML', horizontalalignment='right', verticalalignment='top', transform=ax[1].transAxes, color='black', fontsize=20)
    ax[2].imshow(-gt_depth_copy, aspect='auto', cmap='magma', vmin=-0.8, vmax=-0.05)
    ax[2].axis('off')
    ax[2].text(0.99, 0.95, 'LiDAR', horizontalalignment='right', verticalalignment='top', transform=ax[2].transAxes, color='black', fontsize=20)
    ...
    ax[3].imshow(rf_vis, aspect='auto')
    ax[3].axis('off')
    ax[3].text(0.99, 0.95, 'After SP', horizontalalignment='right', verticalalignment='top', transform=ax[3].transAxes, color='black', fontsize=20)
    ax[3].set_title('Point Cloud View', fontsize=25, weight='bold')
    ax[4].imshow(pred_vis, aspect='auto')
    ax[4].axis('off')
    ax[4].text(0.99, 0.95, 'After ML', horizontalalignment='right', verticalalignment='top', transform=ax[4].transAxes, color='black', fontsize=20)
    ax[5].imshow(gt_vis, aspect='auto')
    ax[5].axis('off')
    ax[5].text(0.99, 0.95, 'LiDAR', horizontalalignment='right', verticalalignment='top', transform=ax[5].transAxes, color='black', fontsize=20)
